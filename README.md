> [求助]意外发现小米部分设备cit支持抓握手包（设置-内核版本-cit-辅助工具-wlan抓包测试工具），目前发现输出到/data/vendor/wlan_logs/，且提取后确实能用来跑字典。分析apk写的导出会输出到/sdcard/MIUI/debug_log/，但是测试没有发现对应目录，说不定能实现小米免root抓包，如果有知道的请发issue
